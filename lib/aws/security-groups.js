...
class SecurityGroupManager {
  ...
  async createSecurityGroup(config) {
    try {
      ...
      // Use full timestamp + random suffix to ensure uniqueness across all deployments
      // This prevents security group reuse between different deployments
      const timestamp = Date.now();
      const randomSuffix = Math.random().toString(36).substring(2, 8);
      const groupName = `${projectName}-sg-${timestamp}-${randomSuffix}`;
      const description = `Security group for ${projectName} managed by focal-deploy`;

      // IMPORTANT: Do NOT reuse existing security groups
      // Each deployment MUST have its own isolated security group
      // This ensures proper resource isolation in multi-tenant environments
