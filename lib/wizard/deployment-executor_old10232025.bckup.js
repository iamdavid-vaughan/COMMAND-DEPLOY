...
class DeploymentExecutor {
      ...
      if (completeConfig.infrastructure?.ec2Instance?.instanceId && completeConfig.infrastructure?.ec2Instance?.publicIpAddress) {
        try {
          ...
          const sshConnectionParams = {
            ...
            port: completeConfig.infrastructure?.sshPort || 22,
            username: completeConfig.security?.ssh?.deploymentUser || (operatingSystem === 'debian' ? 'admin' : 'ubuntu')
          };
  ...
  async buildCompleteConfig(projectPath, stepData, infrastructureResult) {
    ...
    return {
      ...
      aws: {
        ...savedConfig.aws,
        keyPath: stepData.credentials?.aws?.keyPath || savedConfig.aws?.keyPath
      }
    };
  }
