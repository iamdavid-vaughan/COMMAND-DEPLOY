...
class DeploymentExecutor {
  ...
  async executeSecurityPhase(projectPath, stepData, deploymentState) {
    ...
    // CRITICAL FIX: Check if we're resuming with existing SSH options
    // If SSH hardening was partially completed, we need to use the updated SSH parameters
    const isResuming = deploymentState.sshOptions &&
                       deploymentState.sshOptions.sshHardeningApplied;

    if (isResuming) {
      Logger.info(chalk.yellow(`🔄 Resuming security phase with existing SSH configuration`));
      Logger.info(chalk.gray(`Current SSH port: ${deploymentState.sshOptions.port}, User: ${deploymentState.sshOptions.username}`));

      // Use the saved SSH options from previous run
      const securityResult = await this.securityService.hardenSecurity(
        completeConfig,
        deploymentState.sshOptions
      );

      // Update deployment state with results
      deploymentState.sshOptions = {
        ...deploymentState.sshOptions,
        ...securityResult.sshOptions
      };
      deploymentState.completeConfig = completeConfig;
