...
class DeploymentExecutor {
  ...
  async executeInfrastructurePhase(projectPath, stepData, deploymentState) {
    ...
    // The infrastructureResult should now contain securityGroupId and s3BucketName
    // directly from the UpCommand return value (no need to load from state file)
    Logger.info(chalk.gray(`   Security Group ID from UpCommand: ${infrastructureResult.securityGroupId || 'NOT FOUND'}`));
    Logger.info(chalk.gray(`   S3 Bucket from UpCommand: ${infrastructureResult.s3BucketName || 'NOT FOUND'}`));

    // Fallback: Load from saved state if not in the return value (shouldn't be needed)
    let s3BucketName = infrastructureResult.s3BucketName;
    let securityGroupId = infrastructureResult.securityGroupId;

    if (!securityGroupId || !s3BucketName) {
      Logger.info(chalk.yellow('   Fallback: Loading resource IDs from deployment state file...'));
      ...
      try {
        if (await fs.pathExists(upStatePath)) {
          ...
          s3BucketName = s3BucketName || upState.resources?.s3Bucket?.bucketName;
          securityGroupId = securityGroupId || upState.resources?.securityGroup?.securityGroupId;
  ...
  async executeSecurityPhase(projectPath, stepData, deploymentState) {
    ...
    Logger.info(chalk.gray(`   AWS Region: ${completeConfig.aws?.region || 'NOT FOUND'}`));
    Logger.info(chalk.gray(`   AWS Credentials: ${completeConfig.aws?.accessKeyId ? 'Present (accessKeyId: ' + completeConfig.aws.accessKeyId.substring(0, 8) + '...)' : 'NOT FOUND'}`));
    ...
    const sshOptions = {
      ...
      // CRITICAL FIX: AWS credentials need to be in SDK format {accessKeyId, secretAccessKey}
      credentials: {
        accessKeyId: completeConfig.aws?.accessKeyId || completeConfig.credentials?.aws?.accessKeyId,
        secretAccessKey: completeConfig.aws?.secretAccessKey || completeConfig.credentials?.aws?.secretAccessKey
      },
    ...
    if (securityResult?.ssh?.success && securityResult.ssh.customPort) {
      ...
      Logger.info(chalk.gray(`🔄 SSH parameters updated for subsequent phases: port ${sshOptions.port}, username ${sshOptions.username}, isInitialConnection=${sshOptions.isInitialConnection}`));
    }
  ...
  async updateConfigWithInfrastructure(projectPath, infrastructureResult) {
    ...
    config.infrastructure = {
      ...
      ...(infrastructureResult.securityGroupId && {
        securityGroup: {
          id: infrastructureResult.securityGroupId
        }
      })
    };

    // Also store security group ID at AWS level for backward compatibility
    if (infrastructureResult.securityGroupId) {
      config.aws = config.aws || {};
      config.aws.securityGroupId = infrastructureResult.securityGroupId;
    }
    ...
    if (infrastructureResult.securityGroupId) {
      Logger.info(chalk.gray(`  Security Group ID: ${infrastructureResult.securityGroupId}`));
    }
  }
  ...
  async buildCompleteConfig(projectPath, stepData, infrastructureResult) {
    ...
    // Extract security group ID from multiple possible sources
    const securityGroupId = infrastructureResult?.securityGroupId ||
                            savedConfig.infrastructure?.securityGroup?.id ||
                            savedConfig.aws?.securityGroupId;

    Logger.info(chalk.gray(`Building complete config with EC2: ${instanceId} @ ${publicIp}`));
    Logger.info(chalk.gray(`Security Group ID: ${securityGroupId || 'NOT FOUND'}`));
    ...
    return {
      ...
      infrastructure: {
        ...
        // CRITICAL FIX: Include security group ID for port 22 removal in security phase
        ...(securityGroupId && {
          securityGroup: {
            id: securityGroupId
          }
  ...
  async generateDeploymentInfo(stepData, deploymentResults = {}) {
    ...
    const sshConnection = {
      ...
      // CRITICAL FIX: AWS SSH keys are saved WITHOUT .pem extension
      privateKeyPath: `~/.ssh/${stepData.security?.ssh?.keyPairName || deploymentResults.infrastructure?.keyPairName || 'focal-deploy-keypair'}`
    };
