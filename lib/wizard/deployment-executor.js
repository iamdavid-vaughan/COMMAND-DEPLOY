...
class DeploymentExecutor {
  ...
  async saveWizardConfiguration(projectPath, stepData) {
    ...
    // Load AWS credentials from secure storage
    let awsCredentials = stepData.credentials?.aws;
    if (!awsCredentials || !awsCredentials.accessKeyId) {
      try {
        const storedCreds = await keytar.getPassword('focal-deploy', 'aws');
        if (storedCreds) {
          awsCredentials = JSON.parse(storedCreds);
          Logger.info(chalk.gray('✓ Loaded AWS credentials from secure storage'));
        }
      } catch (error) {
        Logger.warn(chalk.yellow('⚠️  Failed to load saved credentials:'));
      }
    }
