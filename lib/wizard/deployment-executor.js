...
class DeploymentExecutor {
  ...
  async executeSecurityPhase(projectPath, stepData, deploymentState) {
    ...
    // CRITICAL FIX: Initial SSH connection MUST use port 22 for fresh EC2 instances
    // Custom ports (like 9022) are only configured AFTER security hardening
    // Only use custom port if security hardening has already been completed (resume scenario)
    const isSecurityHardeningComplete = deploymentState?.completedPhases?.includes('security') ||
                                        deploymentState?.sshOptions?.port > 22;

    const infrastructureSSHPort = isSecurityHardeningComplete ?
      (deploymentState.sshOptions?.port || deploymentState.deploymentResults?.security?.ssh?.customPort || 22) :
      22; // ALWAYS use default port 22 for initial connection to fresh instances

    const targetCustomPort = completeConfig.securityConfig?.sshHardening?.customPort || 9022;
    ...
    const sshOptions = {
      ...
      port: infrastructureSSHPort,  // Port 22 for fresh instances, custom port for resumed deployments
  ...
  async buildCompleteConfig(projectPath, stepData, infrastructureResult) {
    ...
    return {
      ...
      dnsConfig: stepData.dnsConfig || { enabled: false },
      sslConfig: stepData.sslConfig || { enabled: false },
      applicationConfig: stepData.applicationConfig || { enabled: false },
      aws: {
        ...savedConfig.aws,
        keyPath: infrastructureResult?.keyPath || infrastructureResult?.privateKeyPath || stepData.credentials?.aws?.keyPath || savedConfig.aws?.keyPath
      }
    };
  }
