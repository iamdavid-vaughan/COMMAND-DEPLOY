  ...
  async configureDomains(dnsCredentials, setupMode = 'advanced') {
    ...
    if (dnsCredentials.enabled && dnsCredentials.provider) {
      try {
        ...
        const axios = require('../utils/axios-wrapper');
        ...
        if (availableDomains.length > 0) {
          ...
          if (selectedDomains && selectedDomains.length > 0) {
            ...
            // Ask about wildcard SSL if SSL is enabled
            let useWildcardSSL = false;
            if (enableSSL && selectedDomains && selectedDomains.length > 0) {
              const { useWildcard } = await inquirer.prompt([{
                type: 'confirm',
                name: 'useWildcard',
                message: 'Use wildcard SSL certificate (*.domain) to cover all subdomains automatically?',
                default: true
              }]);
              useWildcardSSL = useWildcard;

              if (useWildcardSSL) {
                console.log(chalk.gray('  ℹ️  Wildcard SSL will generate certificates for domain.com and *.domain.com'));
                console.log(chalk.gray('     This covers all subdomains with a single certificate\n'));
              }
            }
            ...
            for (const primaryDomain of primaryDomainsArray) {
              ...
              // Build SSL-specific domain list
              if (enableSSL) {
                if (useWildcardSSL) {
                  // Wildcard mode: just root domain + wildcard
                  sslDomains.push(primaryDomain);
                  sslDomains.push(`*.${primaryDomain}`);
                } else {
                  // Standard mode: all specific domains
                  sslDomains.push(...domainConfig.domains);
                }
              }
            }
            ...
            if (enableSSL && sslEmail) {
              ...
              if (useWildcardSSL) {
                console.log(chalk.white(`SSL Mode: ${chalk.green('Wildcard')}`));
                console.log(chalk.white(`SSL Domains: ${chalk.gray(sslDomains.join(', '))}`));
              } else {
                console.log(chalk.white(`SSL Mode: ${chalk.yellow('Individual')}`));
                console.log(chalk.white(`SSL Domains: ${chalk.gray(`${sslDomains.length} domain(s)`)}`));
              }
            }
            console.log();
            ...
            return {
              ...
              ssl: {
                enabled: enableSSL,
                email: sslEmail,
                domains: enableSSL ? sslDomains : allDomains,
                useWildcard: useWildcardSSL,
