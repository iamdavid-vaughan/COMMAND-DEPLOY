...
class WizardManager {
  ...
  async resumeWizard(sessionId) {
    try {
      ...
      // Check if wizard is already completed AND all steps were actually executed
      // Fix: Don't return early if currentStep hasn't reached the end
      if (this.wizardState.completed && this.currentStep >= this.steps.length) {
      ...
      // If wizard was marked completed but steps remain, reset the completed flag
      if (this.wizardState.completed && this.currentStep < this.steps.length) {
        this.logger.warn(`⚠️  Wizard was marked completed but stopped at step ${this.currentStep + 1}/${this.steps.length}`);
        this.logger.info('🔄 Resuming remaining steps...');
        this.wizardState.completed = false;
      }
  ...
  async configureSSL() {
    ...
    if (sslProvider.provider === 'letsencrypt') {
      ...
      // Use dnsConfig.domains which already includes ALL base domains + their subdomains
      // This supports multiple base domains (e.g., focuswithfocal.com AND focuswithfocal.io)
      let domains = [];

      if (this.stepData.dnsConfig.domains && this.stepData.dnsConfig.domains.length > 0) {
        // dnsConfig.domains already has all domains built correctly from project-configurator
        domains = this.stepData.dnsConfig.domains;
      } else {
        // Fallback for legacy or manual configuration
        domains = [this.stepData.dnsConfig.primaryDomain];
      ...
      // Add wildcards if challenge type supports it (DNS-01 only)
      if (letsEncryptConfig.challengeType === 'dns-01' && letsEncryptConfig.useWildcard) {
        // Get base domains (not subdomains)
        const baseDomains = this.stepData.dnsConfig.primaryDomains ||
                           [this.stepData.dnsConfig.primaryDomain];
