...
const { selectTemplateInteractive } = require('../commands/templates');
...
class WizardManager {
  ...
  async runWizard(projectName, options = {}) {
    try {
      ...
      // then exit cleanly to prevent hanging
      setTimeout(() => {
        process.exit(0);
      }, 1000);
  ...
  async showWelcome() {
    ...
    try {
      ...
      // Check if license file exists first
      const fs = require('fs-extra');
      if (await fs.pathExists(licenseManager.licenseFile)) {
        const licenseInfo = await licenseManager.loadLicense();

        if (licenseInfo && licenseInfo.licenseKey) {
          const licenseType = licenseInfo.licenseType || 'UNKNOWN';
          const email = licenseInfo.email || 'Not specified';
          const version = require('../../package.json').version;

          // Validate license with server to get current status
          const status = await licenseManager.isLicenseValid();
          const licenseStatus = status.valid ? chalk.green('Active') : chalk.red('Inactive');

          console.log(chalk.yellow('📝 ') + chalk.white(`Version: ${chalk.cyan('v' + version)}`));
          console.log(chalk.yellow('📧 ') + chalk.white(`Licensed to: ${chalk.cyan(email)}`));
          console.log(chalk.yellow('🔑 ') + chalk.white(`License: ${licenseStatus}`));
          console.log(chalk.yellow('👤 ') + chalk.white(`Subscriber: ${chalk.cyan(licenseType)}`));
          console.log();
        }
      }
    } catch (error) {
      // Silently fail if license info can't be retrieved
    }
  ...
  async configureProject() {
    // Template selection (if not already specified via --template flag)
    if (!this.initialOptions.template && !this.stepData.selectedTemplate) {
      console.log(chalk.bold.cyan('\n📦 Deployment Template Selection'));
      console.log(chalk.gray('Choose a pre-configured template to get started quickly, or skip to configure manually.'));
      console.log('');

      try {
        const selectedTemplate = await selectTemplateInteractive(this.stepData.infrastructure?.provider || 'aws');
        if (selectedTemplate) {
          this.stepData.selectedTemplate = selectedTemplate;
          this.logger.success(`✓ Using template: ${chalk.cyan(selectedTemplate.name)}`);
        }
      } catch (error) {
        this.logger.warn('Template selection failed, continuing with manual configuration');
      }
    } else if (this.initialOptions.template && !this.stepData.selectedTemplate) {
      // Template slug was provided via command line flag
      this.logger.info(`📦 Loading template: ${chalk.cyan(this.initialOptions.template)}`);
      // Template will be loaded and applied during project configuration
    }
    ...
    this.stepData.projectConfig = await this.projectConfigurator.configure(
      ...
      this.stepData.selectedTemplate || this.initialOptions.template
    );
  ...
  async configureDNSManually(dnsProvider) {
    ...
    // CRITICAL FIX: Build domains array from primaryDomain and subdomains
    // This is required for SSL certificate generation to include all domains
    ...
    this.stepData.dnsConfig = {
      ...
      subdomains: domainConfig.subdomains,
      domains: domains,  // CRITICAL: Add domains array
      credentials: this.stepData.credentials.dns
    };
  }
    ...
    if (sslProvider.provider === 'letsencrypt') {
      const letsEncryptQuestions = [
        ...
        },
        {
          type: 'confirm',
          name: 'useStaging',
          message: 'Use Let\'s Encrypt STAGING mode? (For testing - certificates won\'t be trusted by browsers)',
          default: false,
          when: () => true // Always show this option
        }
      ];

      const letsEncryptConfig = await inquirer.prompt(letsEncryptQuestions);

      if (letsEncryptConfig.useStaging) {
        console.log(chalk.yellow('\n⚠️  STAGING MODE ENABLED'));
        console.log(chalk.yellow('   Certificates will be issued by Let\'s Encrypt staging server'));
        console.log(chalk.yellow('   These certificates are NOT trusted by browsers (for testing only)'));
        console.log(chalk.yellow('   This helps avoid production rate limits while testing\n'));
      }

      // Generate certificate domains list from ALL configured domains
      // CRITICAL FIX: Use dnsConfig.domains array which includes ALL base domains and subdomains
      // Previous bug: only used primaryDomain, ignoring second domain (e.g., focuswithfocal.io)
      const domains = this.stepData.dnsConfig.domains || [];

      // Fallback to building from primaryDomain if domains array not available
      if (domains.length === 0) {
      ...
      this.stepData.sslConfig = {
        ...
        domains: domains,
        autoRenewal: true,
        useStaging: letsEncryptConfig.useStaging || false
      };

      // Include DNS provider credentials for automatic DNS-01 challenges
      // This enables automatic TXT record creation via DNS provider APIs
      if (letsEncryptConfig.challengeType === 'dns-01' &&
          this.stepData.credentials?.dns &&
          this.stepData.credentials.dns.enabled &&
          this.stepData.credentials.dns.provider !== 'manual' &&
          this.stepData.credentials.dns.provider !== 'skip') {

        // Extract credentials (excluding metadata fields)
        const { provider, validated, enabled, ...credentials } = this.stepData.credentials.dns;

        this.stepData.sslConfig.dnsProvider = {
          name: provider,
          credentials: credentials
        };

        console.log(chalk.green(`✓ Configured automatic DNS-01 challenges using ${provider}`));
      }
    } else {
      this.stepData.sslConfig = {
        enabled: false,
