...
class WizardManager {
  ...
  async configureSSL() {
    ...
    if (sslProvider.provider === 'letsencrypt') {
      ...
      // Use domain detection service to properly handle wildcard conflicts
      const domainDetectionService = new DomainDetectionService();
      
      // Build a temporary config object for domain detection
      const tempConfig = {
        domain: {
          primary: this.stepData.dnsConfig.primaryDomain,
          subdomains: this.stepData.dnsConfig.subdomains || []
        }
      };
      
      // Detect domains and filter out redundant ones (e.g., specific subdomains when wildcards exist)
      const detectionResult = await domainDetectionService.detectDomains(tempConfig, {
        includeWildcards: false, // Don't automatically add wildcards unless explicitly configured
        noWww: false
      });
      
      console.log(chalk.blue(`📋 Detected domains for SSL: ${detectionResult.allDomains.join(', ')}`));
      
      // Validate domains to ensure no wildcard conflicts
      const validation = domainDetectionService.validateDomains(detectionResult);
      if (validation.warnings.length > 0) {
        console.log(chalk.yellow('⚠️  Domain validation warnings:'));
      ...
      this.stepData.sslConfig = {
        ...
        domains: detectionResult.allDomains, // Use filtered domains from detection service
        autoRenewal: true
      };
    } else {
      this.stepData.sslConfig = {
