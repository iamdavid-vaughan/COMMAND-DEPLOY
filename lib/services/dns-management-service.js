...
class DNSManagementService {
  ...
  async setupDNSRecords(config, dryRun = false) {
    ...
    try {
      let { provider, domains } = dnsConfig;

      // CRITICAL FIX: Normalize provider format
      // Wizard stores provider as string, but service expects object with name property
      if (typeof provider === 'string') {
        provider = {
          name: provider,
          credentials: dnsConfig.credentials
        };
      }
