    ...
    try {
      ...
      if (securityConfig.sshHardening?.enabled) {
        ...
        if (hardeningResults.ssh?.success && hardeningResults.ssh?.customPort) {
          ...
          sshOptions.privateKeyPath = hardeningResults.ssh.privateKeyPath;  // CRITICAL FIX: Include private key path
          sshOptions.sshHardeningApplied = true; // CRITICAL FIX: Set the hardening flag
          sshOptions.isInitialConnection = false; // CRITICAL FIX: Never use port 22 again after hardening

          // CRITICAL FIX: Disconnect old port 22 connection to prevent any reuse attempts
          // After SSH hardening, port 22 is closed, so we should never try to use that connection again
          logger.info(chalk.gray('Clearing old SSH connection on port 22 from connection pool'));
          this.sshService.disconnect(host, 22);

          logger.info(chalk.gray(`SSH options updated for subsequent steps: port ${sshOptions.port}, username ${sshOptions.username}, privateKeyPath ${sshOptions.privateKeyPath ? 'present' : 'missing'}, sshHardeningApplied=${sshOptions.sshHardeningApplied}, isInitialConnection=${sshOptions.isInitialConnection}`));
        }
      }
  ...
  async configureSSHHardening(instanceId, host, sshConfig, sshOptions = {}, dryRun = false) {
    ...
    try {
      ...
      const connectionParams = {
        ...
        sshHardeningApplied: false, // Initial state - use configured values for initial connection
        infrastructureSSHPort: sshOptions.port // Pass the infrastructure's SSH port for initial connection
      };

      // Generate or get existing SSH public key for deployment user creation
      let publicKey = null;
      let discoveredPrivateKeyPath = null;
      try {
        // CRITICAL FIX: Use the SSH key from infrastructure phase if provided
        // Only discover existing keys as a fallback
        if (sshOptions.privateKeyPath) {
          discoveredPrivateKeyPath = sshOptions.privateKeyPath;
          logger.info(`Using SSH private key from infrastructure: ${discoveredPrivateKeyPath}`);
        } else {
          // Fallback: Try to find existing key (prioritize state keys for fresh instances)
          const existingKeyPath = await this.securityManager.getExistingPrivateKeyPath(true);
          discoveredPrivateKeyPath = existingKeyPath;
          logger.info(`Found SSH private key: ${discoveredPrivateKeyPath}`);
        }

        const publicKeyPath = `${discoveredPrivateKeyPath}.pub`;
        ...
        if (await fs.pathExists(publicKeyPath)) {
          ...
          if (!publicKey.match(/^ssh-(rsa|ed25519|ecdsa-sha2-\w+)\s+[A-Za-z0-9+/]+=*\s*/)) {
            ...
            try {
              ...
              const regeneratedKey = execSync(`ssh-keygen -y -f "${discoveredPrivateKeyPath}"`, { encoding: 'utf8' }).trim();
      ...
      // Update connectionParams with discovered private key path
      if (discoveredPrivateKeyPath) {
        connectionParams.privateKeyPath = discoveredPrivateKeyPath;
        logger.info(`Using discovered SSH private key: ${discoveredPrivateKeyPath}`);
      }
      ...
      return {
        ...
        privateKeyPath: discoveredPrivateKeyPath || connectionParams.privateKeyPath,  // Include private key path
        keyAuthOnly: true,
  ...
  async configureAutoUpdates(host, updateConfig, sshOptions = {}, dryRun = false) {
    ...
    try {
      const updateCommands = [
        ...
        // Configure automatic updates (non-interactive)
        // Note: dpkg-reconfigure removed - we configure directly via files below
...
EOF`,
        ...
        // Configure rkhunter BEFORE running update
        // Fix: rkhunter's default WEB_CMD="/bin/false" is invalid
        // Set WEB_CMD to empty string to disable web updates (cleaner than "NONE")
        'sudo sed -i \'s|^WEB_CMD=.*|WEB_CMD=""|g\' /etc/rkhunter.conf',
        // Also update MIRRORS_MODE to 0 (local only) to avoid web dependency
        'sudo sed -i \'s|^#MIRRORS_MODE=.*|MIRRORS_MODE=0|g\' /etc/rkhunter.conf',
        'sudo sed -i \'s|^MIRRORS_MODE=.*|MIRRORS_MODE=0|g\' /etc/rkhunter.conf',

        // Now safe to run rkhunter update and property update
        'sudo rkhunter --propupd',  // Only update file properties, no web access needed
