...
class SecurityHardeningService {
  ...
  async configureSSHHardening(instanceId, host, sshConfig, sshOptions = {}, dryRun = false) {
    ...
    try {
      ...
      // CRITICAL FIX: Use the privateKeyPath from sshOptions (passed from infrastructure)
      // instead of searching for any key, which might find the wrong one
      let publicKey = null;
      try {
        // Use the specific private key path that was used to create the infrastructure
        const existingKeyPath = sshOptions.privateKeyPath || connectionParams.privateKeyPath ||
                                await this.securityManager.getExistingPrivateKeyPath();

        logger.info(`🔑 Using private key for public key extraction: ${existingKeyPath}`);
        ...
        if (await fs.pathExists(publicKeyPath)) {
          ...
          if (publicKey) {
            ...
            logger.info(chalk.gray(`   Key: ${existingKeyPath}`));
            logger.info(chalk.gray(`   Public key file: ${publicKeyPath}`));
          }
        }
      } catch (error) {
  ...
  async configureAutoUpdates(host, updateConfig, sshOptions = {}, dryRun = false) {
    ...
    try {
      const updateCommands = [
        ...
        // Configure automatic updates non-interactively
...
EOF`,
        ...
        // Clean up any existing AIDE databases to avoid interactive prompts
        'sudo rm -f /var/lib/aide/aide.db /var/lib/aide/aide.db.new',
        'sudo aideinit',
        'sudo mv /var/lib/aide/aide.db.new /var/lib/aide/aide.db',

        // Configure rkhunter
        // NOTE: Skip rkhunter --update on Debian systems as the package manager handles updates
        // The default Debian config has WEB_CMD="/bin/false" to disable online updates
        // 'sudo rkhunter --update',  // Disabled - causes "Invalid WEB_CMD" error on Debian
        'sudo rkhunter --propupd',  // Update file properties database (safe on all systems)
