...
class SecurityHardeningService {
  ...
  async configureSSHHardening(instanceId, host, sshConfig, sshOptions = {}, dryRun = false) {
    ...
    try {
      ...
      try {
        // CRITICAL FIX: Use the SSH key passed from Phase 1 infrastructure setup
        // Only discover a key if none was provided from previous phase
        const keyPathToUse = sshOptions.privateKeyPath || await this.securityManager.getExistingPrivateKeyPath();
        discoveredPrivateKeyPath = keyPathToUse;  // Save for later use
        const publicKeyPath = `${keyPathToUse}.pub`;
...
EOF`,
        ...
        // Initialize AIDE database (remove old one first to avoid prompts)
        'sudo rm -f /var/lib/aide/aide.db.new /var/lib/aide/aide.db',
        'sudo aideinit',
        'sudo mv /var/lib/aide/aide.db.new /var/lib/aide/aide.db',

        // Fix rkhunter WEB_CMD configuration before running update
        'sudo sed -i \'s|^WEB_CMD=.*|WEB_CMD=""|g\' /etc/rkhunter.conf',
        'sudo sed -i \'s|^#DISABLE_TESTS=.*|DISABLE_TESTS="suspscan hidden_procs deleted_files packet_cap_apps apps"|g\' /etc/rkhunter.conf',

        // Configure rkhunter (allow update to fail since online updates are optional)
