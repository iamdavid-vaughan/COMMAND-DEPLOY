    ...
    try {
      ...
      if (securityConfig.sshHardening?.enabled) {
        ...
        if (hardeningResults.ssh?.success && hardeningResults.ssh?.customPort) {
          ...
          logger.info(chalk.gray(`SSH options updated for subsequent steps: port ${sshOptions.port}, username ${sshOptions.username}, privateKeyPath ${sshOptions.privateKeyPath ? 'present' : 'missing'}, sshHardeningApplied=${sshOptions.sshHardeningApplied}`));
        }
      }
