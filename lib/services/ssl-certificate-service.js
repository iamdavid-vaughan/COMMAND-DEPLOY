...
class SSLCertificateService {
  constructor(sshService) {
    // Create SSHService if not provided (for compatibility with DeploymentExecutor)
  ...
  async generateCertificates(host, sslConfig, dnsConfig, sshOptions = {}, dryRun = false) {
    ...
    // Group domains by base domain (TLD) - each base domain needs its own certificate
    const domainGroups = this.groupDomainsByBaseDomain(domains);

    logger.info(chalk.blue(`📋 Found ${domainGroups.length} base domain(s) requiring separate certificates`));
    domainGroups.forEach(group => {
      logger.info(chalk.gray(`   - ${group.baseDomain}: ${group.domains.length} domain(s)`));
    });
    ...
    try {
      ...
      // Generate a separate certificate for each base domain group
      for (const group of domainGroups) {
        logger.info(chalk.blue(`\n🔐 Generating certificate for ${group.baseDomain}...`));

        const groupDomainConfigs = certDomainConfigs.filter(dc =>
          group.domains.includes(dc.domain)
        );
        ...
        if (group.domains.length === 1) {
          ...
          const challengeMethod = groupDomainConfigs[0].challengeMethod;
          ...
          // Multi-domain SAN certificate for this base domain
        ...
        certificateResult.baseDomain = group.baseDomain;
        certificateResults.push(certificateResult);

        logger.success(chalk.green(`✅ Certificate generated for ${group.baseDomain}`));
      }

      logger.success(chalk.green(`✅ All SSL certificates generated successfully`));

      // Return the first certificate result for backward compatibility
      // but include all results in the certificates array
      return {
        ...certificateResults[0],
        certificates: certificateResults,
        multiCertificate: certificateResults.length > 1
      };
  ...
  /**
   * Group domains by their base domain (TLD)
   * Example: focuswithfocal.com, www.focuswithfocal.com, *.focuswithfocal.com -> group 1
   *          focuswithfocal.io, api.focuswithfocal.io, *.focuswithfocal.io -> group 2
   */
  groupDomainsByBaseDomain(domains) {
    const groups = {};

    for (const domain of domains) {
      // Extract base domain (handle wildcards)
      let baseDomain;
      ...
      groups[baseDomain].domains.push(domain);
    }
  ...
  async configureNginxSSL(host, sslConfig, applicationConfig, certificateResult, sshOptions = {}, dryRun = false) {
    ...
    // Debug logging
    logger.info(chalk.gray(`🔍 Debug: sslConfig.domains.length = ${domains.length}`));
    logger.info(chalk.gray(`🔍 Debug: multiCertificate = ${certificateResult.multiCertificate || false}`));

    try {
      // Generate DH parameters first (required for all SSL configs)
      ...
      if (certificateResult.multiCertificate && certificateResult.certificates) {
        // Multiple base domains - create separate server blocks for each
        logger.info(chalk.blue(`📍 Configuring ${certificateResult.certificates.length} SSL certificates`));

        for (const cert of certificateResult.certificates) {
          logger.info(chalk.blue(`   Configuring ${cert.baseDomain}...`));

          // Get domains for this base domain
          const baseDomainGroup = this.groupDomainsByBaseDomain(domains).find(g => g.baseDomain === cert.baseDomain);
          const baseDomains = baseDomainGroup ? baseDomainGroup.domains : cert.domains;

          const domainConfigs = baseDomains.map(domain => ({ domain }));

          // Generate and apply Nginx configuration for this base domain
          ...
          const configPath = `/etc/nginx/sites-available/${cert.baseDomain}`;
          ...
          logger.success(chalk.green(`   ✅ ${cert.baseDomain} configured`));
        }
        ...
        if (domains.length === 1) {
          ...
          logger.info(chalk.blue('📍 Using single domain SSL configuration path'));
          const nginxConfig = this.sslService.generateNginxSSLConfig(
            domains[0],
            appPort,
            certificatePath,
            privateKeyPath
          ...
          const configPath = `/etc/nginx/sites-available/${domains[0]}`;
          await this.sshService.executeCommand(
            host,
            `sudo tee ${configPath} > /dev/null << 'EOF'\n${nginxConfig}\nEOF`,
            sshOptions
          ...
          // Multi-domain configuration with single certificate
          logger.info(chalk.blue('📍 Using multi-domain SSL configuration path'));
  ...
  async setupCertificateRenewal(host, sslConfig, dnsConfig, sshOptions = {}, dryRun = false) {
    ...
    try {
      ...
      const renewalSetup = await this.sslService.setupSSLRenewal(
        host,
        sshOptions,
        dryRun,
        { ssl: sslConfig, dns: dnsConfig }
      ...
      // Only test renewal if not using manual certificates
      // Manual DNS-01 certificates require interactive input and cannot be tested automatically
      const hasManualCerts = renewalSetup.certificateInfo?.hasManualCertificates || false;

      if (!hasManualCerts) {
        logger.info(chalk.blue('🧪 Testing automatic renewal process...'));
        await this.sshService.executeCommand(
          host,
        ...
        logger.info(chalk.yellow('⚠️  Skipping renewal dry-run test (manual certificates detected)'));
      }
