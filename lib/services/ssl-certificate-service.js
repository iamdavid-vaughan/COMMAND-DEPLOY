...
class SSLCertificateService {
  ...
  async configureNginxSSL(host, sslConfig, applicationConfig, certificateResult, sshOptions = {}, dryRun = false) {
    ...
    logger.info(chalk.gray(`🔍 Debug: sslConfig.domains = ${JSON.stringify(domains)}`));
    ...
    try {
      if (domains.length === 1) {
        ...
        // Multi-domain configuration - use the complete method that generates DH params
        logger.info(chalk.blue('📍 Using multi-domain SSL configuration path'));
        const domainConfigs = domains.map(domain => ({ domain }));
        await this.enhancedSSLService.configureMultiDomainNginxSSL(
          host,
          domainConfigs,
          appPort,
          certificatePath,
