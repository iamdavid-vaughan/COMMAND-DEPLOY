...
class SecurityManager {
  ...
  async deploySSHKeysWithState(instanceId, publicIp, publicKey, username = 'deploy', connectionParams) {
    ...
    try {
      ...
      const connection = await ssh.connect(publicIp, {
        ...connectOptions,
        privateKeyPath: connectOptions.privateKeyPath, // Explicitly pass the private key path
        isInitialConnection: !connectionParams.sshHardeningApplied,
...
`;
  ...
  async getExistingPrivateKeyPath() {
    try {
      ...
      // Try multiple possible key path locations
      const possiblePaths = [
        state.aws?.keyPairPath,
        state.resources?.sshKey?.privateKeyPath,
        path.join(require('os').homedir(), '.ssh', state.resources?.sshKey?.keyPairName),
        path.join(require('os').homedir(), '.ssh', `${state.resources?.sshKey?.keyPairName}.pem`),
        // Also check the .focal-deploy/keys directory
        path.join(process.cwd(), '.focal-deploy', 'keys', state.resources?.sshKey?.keyPairName),
        // Check for keys with timestamp suffix in .focal-deploy/keys
        ...(await this.findKeysInFocalDeployDir())
      ].filter(Boolean);
