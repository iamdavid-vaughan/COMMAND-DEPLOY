...
class DNSManager {
  ...
  /**
   * Clean up all _acme-challenge TXT records across all domains
   * This is useful to fix conflicts when Let's Encrypt finds old/stale challenge records
   */
  async cleanupAcmeChallengeRecords(specificDomain = null) {
    try {
      logger.info(chalk.blue('🧹 Cleaning up _acme-challenge TXT records...'));

      const domainsToClean = specificDomain ? [specificDomain] : (await this.getAllDomains()).map(d => d.name);
      const deletedRecords = [];
      let totalFound = 0;

      for (const domainName of domainsToClean) {
        try {
          const records = await this.getDNSRecords(domainName);

          // Find all TXT records with _acme-challenge prefix
          const acmeRecords = records.filter(record =>
            record.type === 'TXT' &&
            (record.name.startsWith('_acme-challenge') || record.name === '_acme-challenge')
          );

          if (acmeRecords.length > 0) {
            totalFound += acmeRecords.length;
            logger.info(chalk.cyan(`   Found ${acmeRecords.length} _acme-challenge record(s) in ${domainName}`));

            for (const record of acmeRecords) {
              try {
                await this.deleteDNSRecord(domainName, record.id);
                const fullName = record.name === '@' ? domainName : `${record.name}.${domainName}`;
                logger.success(chalk.green(`   ✅ Deleted: ${fullName} (TXT: ${record.data.substring(0, 30)}...)`));
                ...
                logger.error(chalk.red(`   ❌ Failed to delete record ${record.id}: ${error.message}`));
              }
            }
          }
        } catch (error) {
          logger.warn(chalk.yellow(`⚠️  Could not scan domain ${domainName}: ${error.message}`));
        }
      }

      if (totalFound > 0) {
        logger.success(chalk.green(`✅ Cleaned up ${deletedRecords.length}/${totalFound} _acme-challenge records`));
      } else {
        logger.info(chalk.gray('ℹ️  No _acme-challenge records found'));
      }
      ...
      logger.error(chalk.red(`❌ Failed to clean up ACME records: ${error.message}`));
      return {
        success: false,
        error: error.message,
        totalFound: 0,
