...
class DomainDetectionService {
  ...
  _processSmartDefaults(config, options = {}) {
    ...
    // Add subdomains if configured
    if (config.domain.subdomains && Array.isArray(config.domain.subdomains)) {
      for (const subdomain of config.domain.subdomains) {
        if (subdomain === '*') {
          // Handle wildcard subdomain
          const wildcardDomain = `*.${this.primaryDomain}`;
          this.wildcardDomains.push(wildcardDomain);
          logger.info(chalk.yellow(`🌟 Wildcard subdomain: ${wildcardDomain}`));
        } else if (subdomain.startsWith('*.')) {
          // Handle wildcard patterns like *.api
          const wildcardDomain = `${subdomain}.${this.primaryDomain}`;
          this.wildcardDomains.push(wildcardDomain);
          logger.info(chalk.yellow(`🌟 Wildcard subdomain: ${wildcardDomain}`));
        } else {
          // Handle regular subdomains
          const subdomainFull = `${subdomain}.${this.primaryDomain}`;
          this.detectedDomains.push(subdomainFull);
          logger.info(chalk.blue(`📎 Subdomain: ${subdomainFull}`));
        }
      }
    }
