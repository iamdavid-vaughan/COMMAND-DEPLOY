    ...
    try {
      ...
      for (const block of certBlocks) {
        ...
        try {
          ...
          const configContent = configResult.stdout;

          // Determine if certificate is manual or automatic
          // Manual certificates have: authenticator = manual
          // Automatic DNS-01 certificates have: authenticator = dns-<provider> (e.g., dns-cloudflare, dns-digitalocean)
          const isManualAuthenticator = configContent.includes('authenticator = manual');
          const isAutomaticDNSAuthenticator = /authenticator = dns-\w+/.test(configContent);

          // A certificate is considered manual if:
          // 1. It explicitly uses "authenticator = manual"
          // 2. It's NOT using an automatic DNS authenticator plugin
          if (isManualAuthenticator && !isAutomaticDNSAuthenticator) {
            ...
            // Certificate is automatic (HTTP-01, or automatic DNS-01 with plugin)
