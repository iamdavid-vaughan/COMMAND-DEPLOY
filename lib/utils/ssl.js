...
class SSLService {
  ...
  async configureNginxSSL(host, domain, appPort, certificatePath, privateKeyPath, sshOptions = {}, dryRun = false) {
    ...
    try {
      // Generate DH parameters for better security with fallback
      const hasDhparam = await this.ensureDhparamFile(host, sshOptions);
      
      const nginxConfig = this.generateNginxSSLConfig(domain, appPort, certificatePath, privateKeyPath, hasDhparam);
  ...
  generateNginxSSLConfig(domain, appPort, certificatePath, privateKeyPath, hasDhparam = false) {
...
${dhparamLine}
  ...
  async setupSSLRenewal(host, sshOptions = {}, dryRun = false, config = null) {
    ...
    try {
      ...
      await this.ensureDhparamFile(host, sshOptions);
