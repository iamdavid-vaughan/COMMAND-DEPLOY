...
`;
  ...
  async getExistingPrivateKeyPath() {
    try {
      ...
      // Build list of possible paths, only adding valid ones
      const possiblePaths = [];
      
      // Direct path from state
      if (state.aws?.keyPairPath) {
        possiblePaths.push(state.aws.keyPairPath);
      }
      
      if (state.resources?.sshKey?.privateKeyPath) {
        possiblePaths.push(state.resources.sshKey.privateKeyPath);
      }
      
      // Paths based on keyPairName (only if keyPairName exists)
      ...
      // Check for keys with timestamp suffix in ~/.ssh/
      const focalDeployKeys = await this.findKeysInFocalDeployDir();
      possiblePaths.push(...focalDeployKeys);
  ...
  /**
   * Find SSH keys in the ~/.ssh/ directory
   */
