...
class SecurityManager {
  ...
  async configureSSHHardeningWithState(instanceId, publicIp, options = {}, connectionParams) {
    ...
    try {
      ...
      // The infrastructure may already be configured for a custom port (like 9022) from the start
      // RESUME FIX: When resuming, we don't know if hardening was partially completed, so try custom port first
      ...
      const connectOptions = {
        ...
        port: connectionParams.sshHardeningApplied ?
          ...
          (connectionParams.infrastructureSSHPort || 22), // Use infrastructure's configured port
        operatingSystem: operatingSystem,
        // Add fallback port to try if primary port fails (for resume scenarios)
        fallbackPort: connectionParams.sshHardeningApplied ? 22 : customPort,
        // Add fallback username to try if primary user fails (for resume scenarios where user was created)
        fallbackUsername: connectionParams.sshHardeningApplied ? this.getSSHUsername(operatingSystem) : username
      };
      ...
      // If no private key path provided, try to discover it
      // For initial connections: Prioritize AWS EC2 key from state
      // For hardened connections: Prioritize focal-deploy keys from ~/.ssh
      if (!connectOptions.privateKeyPath) {
        try {
          const prioritizeAWSKey = !connectionParams.sshHardeningApplied;
          connectOptions.privateKeyPath = await this.getExistingPrivateKeyPath(prioritizeAWSKey);
          deployLogger.logDeploymentStep('SSH_KEY_DISCOVERY', `Found key: ${connectOptions.privateKeyPath}`);
          this.logger.info(`🔑 Using private key: ${connectOptions.privateKeyPath}`);
        } catch (error) {
          deployLogger.logError(error, 'SSH_KEY_DISCOVERY');
          this.logger.error('❌ No SSH private key found. Checked:');
          this.logger.error('  - State file (aws.keyPairPath, resources.sshKey.privateKeyPath)');
          this.logger.error('  - ~/.ssh/focal-deploy-* keys');
          this.logger.error('  - ~/.ssh/[keyPairName] from state');
          this.logger.error(`Actual error: ${error.message}`);
          throw new Error('SSH private key not found. Please ensure AWS EC2 key pair path is configured.');
        }
      }
...
`;
   ...
   * @param {boolean} prioritizeStateKeys - If true, check state paths before focal-deploy keys (for initial connections)
   */
  async getExistingPrivateKeyPath(prioritizeStateKeys = false) {
    try {
      const state = await this.stateManager.loadState();
      
      // For initial connections to fresh EC2 instances, prioritize AWS key pair from state
      if (prioritizeStateKeys) {
        // Priority 1: Check state paths first (AWS EC2 key pair)
        const statePaths = [
        ...
        for (const keyPath of statePaths) {
          if (await fs.pathExists(keyPath)) {
            this.logger.info(`Found SSH private key from state: ${keyPath}`);
            return keyPath;
          }
        }
        
        // Priority 2: Try constructing path from keyPairName if available
        ...
        if (keyPairName) {
          ...
          for (const keyPath of constructedPaths) {
            if (await fs.pathExists(keyPath)) {
              this.logger.info(`Found SSH private key from keyPairName: ${keyPath}`);
              return keyPath;
            }
          }
        }
        
        // Priority 3: Check focal-deploy keys as fallback
        const sshDirKeys = await this.findKeysInSSHDir();
        for (const keyPath of sshDirKeys) {
          if (await fs.pathExists(keyPath)) {
            this.logger.info(`Found focal-deploy SSH private key: ${keyPath}`);
            return keyPath;
          }
        }
      } else {
        // Default behavior: Check focal-deploy keys first (for post-hardening connections)
  ...
  async findKeysInSSHDir() {
    try {
      ...
      return files
        // **FIX: Search for keys with any name pattern, not just focal-deploy-***
        .filter(file => file.includes('keypair') && !file.endsWith('.pub') && !file.endsWith('.pub~'))
  ...
  async waitForInstanceReady(publicIp, connectOptions, deployLogger, maxRetries = 10, baseDelay = 10000) {
    ...
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        // RESUME FIX: If we get authentication failures and have a fallback username, try it after 3 attempts
        if (attempt === 3 &&
            connectOptions.fallbackUsername &&
            connectOptions.fallbackUsername !== connectOptions.username &&
            (error.message.includes('authentication') || error.message.includes('auth'))) {

          this.logger.warn(`⚠️  Username ${connectOptions.username} failed 3 times. Trying fallback username ${connectOptions.fallbackUsername}...`);
          deployLogger.logDeploymentStep('SSH_FALLBACK_USERNAME', `Trying username ${connectOptions.fallbackUsername} after ${connectOptions.username} failed`);

          try {
            const fallbackOptions = { ...connectOptions, username: connectOptions.fallbackUsername };
            const connection = await ssh.connect(publicIp, {
              ...fallbackOptions,
              timeout: 15000
            });
            ...
            if (testResult.stdout && testResult.stdout.includes('SSH connection ready')) {
              this.logger.success(`✅ SSH connection established with fallback username ${connectOptions.fallbackUsername}`);
              deployLogger.logSSHConnection(publicIp, connectOptions.port, connectOptions.fallbackUsername, 'ESTABLISHED',
                `Connected with fallback username`);
              // Update connectOptions to use the working username for remaining attempts
              connectOptions.username = connectOptions.fallbackUsername;
              return true;
            }

            await ssh.disconnect();
          } catch (fallbackError) {
            this.logger.error(`❌ Fallback username ${connectOptions.fallbackUsername} also failed: ${fallbackError.message}`);
            await ssh.disconnect();
          }
        }
    ...
    // If we get here, all attempts failed on primary port
    // RESUME FIX: Try fallback port if configured (for resume scenarios where port may have changed)
    if (connectOptions.fallbackPort && connectOptions.fallbackPort !== connectOptions.port) {
      this.logger.warn(`⚠️  Primary port ${connectOptions.port} failed. Trying fallback port ${connectOptions.fallbackPort}...`);
      deployLogger.logDeploymentStep('SSH_FALLBACK_PORT', `Trying port ${connectOptions.fallbackPort} after port ${connectOptions.port} failed`);

      try {
        const fallbackOptions = { ...connectOptions, port: connectOptions.fallbackPort };
        const connection = await ssh.connect(publicIp, {
          ...fallbackOptions,
          timeout: 15000
        });
        ...
        if (testResult.stdout && testResult.stdout.includes('SSH connection ready')) {
          this.logger.success(`✅ SSH connection established on fallback port ${connectOptions.fallbackPort}`);
          deployLogger.logSSHConnection(publicIp, connectOptions.fallbackPort, connectOptions.username, 'ESTABLISHED',
            `Connected on fallback port`);
          // Update connectOptions to use the working port
          connectOptions.port = connectOptions.fallbackPort;
          return true;
        }

        await ssh.disconnect();
      } catch (fallbackError) {
        this.logger.error(`❌ Fallback port ${connectOptions.fallbackPort} also failed: ${fallbackError.message}`);
        await ssh.disconnect();
      }
    }

    // RESUME FIX: Try fallback username if configured (for resume scenarios where user was created)
    if (connectOptions.fallbackUsername && connectOptions.fallbackUsername !== connectOptions.username) {
      this.logger.warn(`⚠️  Username ${connectOptions.username} failed. Trying fallback username ${connectOptions.fallbackUsername}...`);
      deployLogger.logDeploymentStep('SSH_FALLBACK_USERNAME', `Trying username ${connectOptions.fallbackUsername} after ${connectOptions.username} failed`);
      ...
      try {
        ...
        if (testResult.stdout && testResult.stdout.includes('SSH connection ready')) {
          ...
          deployLogger.logSSHConnection(publicIp, connectOptions.port, connectOptions.fallbackUsername, 'ESTABLISHED',
            `Connected with fallback username`);
          // Update connectOptions to use the working username
          connectOptions.username = connectOptions.fallbackUsername;
          return true;
        }
    ...
    // If we get here, all attempts and fallbacks failed
    const errorMessage = `SSH connection failed after ${maxRetries} attempts on port ${connectOptions.port}${connectOptions.fallbackPort ? ` and fallback port ${connectOptions.fallbackPort}` : ''}${connectOptions.fallbackUsername ? ` with usernames ${connectOptions.username} and ${connectOptions.fallbackUsername}` : ''}. Last error: ${lastError?.message || 'Unknown error'}`;
   ...
   */
  async testNewSSHConnection(publicIp, username, customPort, privateKeyPath, deployLogger = null) {
    const ssh = new SSHService();
