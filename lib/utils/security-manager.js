...
class SecurityManager {
    ...
    try {
      ...
      try {
        ...
        if (options.securityGroupId) {
          ...
          this.logger.success('✅ Port 22 access removed from security group');
        } else {
          this.logger.error('❌ SECURITY WARNING: Security Group ID not provided - port 22 was NOT removed!');
          this.logger.warn('⚠️  Port 22 remains open in your AWS security group');
          this.logger.info('💡 Run "focal-deploy close-port-22" from your project directory to close it manually');
        }
      } catch (error) {
        this.logger.error('❌ SECURITY WARNING: Failed to remove port 22 from security group:', error.message);
        this.logger.warn('⚠️  Port 22 remains open in your AWS security group - this is a security risk!');
        this.logger.info('💡 Run "focal-deploy close-port-22" from your project directory to close it manually');
  ...
  async configureSSHHardeningWithState(instanceId, publicIp, options = {}, connectionParams) {
    ...
    try {
      ...
      try {
        // CRITICAL FIX: Remove port 22 from UFW FIRST (using existing connection on port 9022)
        // Then remove from AWS Security Group
        // Previous bug: Removed from AWS first, which blocked port 22, preventing UFW cleanup

        this.logger.info('🛡️ Step 1: Removing port 22 from UFW firewall...');
        await this.removePort22FromFirewall(connection);
        this.logger.success('✅ Port 22 removed from UFW firewall');
        deployLogger.logFirewallChange('REMOVE_PORT', 22, 'SUCCESS', 'Port 22 removed from UFW');

        this.logger.info('🔒 Step 2: Removing port 22 from AWS Security Group...');
        const SecurityGroupManager = require('../aws/security-groups');
        const securityGroupManager = new SecurityGroupManager(options.region, options.credentials);
        ...
        if (options.securityGroupId) {
          ...
          this.logger.warn('⚠️  Port 22 remains open in your AWS security group');
          this.logger.info('💡 Run "focal-deploy close-port-22" from your project directory to close it manually');
          deployLogger.logError(new Error('Security Group ID not provided'), 'PORT_22_REMOVAL');
        }

        this.logger.success('✅ Phase 6 Complete: Port 22 access removed from both UFW and AWS');
      } catch (error) {
        this.logger.error('❌ SECURITY WARNING: Failed to remove port 22 access:', error.message);
        this.logger.warn('⚠️  Port 22 may still be open - this is a security risk!');
        this.logger.info('💡 Run "focal-deploy close-port-22" from your project directory to close it manually');
...
`;
  ...
  async configureFail2ban(instanceId, publicIp, options = {}) {
    ...
    try {
      ...
      // CRITICAL FIX: Wait for dpkg locks (Ubuntu unattended-upgrades often runs on boot)
      this.logger.info('Waiting for package manager locks to be released...');
      let retries = 0;
      const maxRetries = 12; // 12 * 10 seconds = 2 minutes max wait
      while (retries < maxRetries) {
        try {
          // Check if dpkg is locked
          const lockCheck = await ssh.exec('sudo lsof /var/lib/dpkg/lock-frontend 2>/dev/null || echo "unlocked"');
          if (lockCheck.stdout.includes('unlocked')) {
            this.logger.success('Package manager is ready');
            break;
          }

          retries++;
          this.logger.info(`Package manager is locked, waiting... (attempt ${retries}/${maxRetries})`);
          await new Promise(resolve => setTimeout(resolve, 10000)); // Wait 10 seconds
        } catch (error) {
          // lsof command failed, assume unlocked
          break;
        }
      }
