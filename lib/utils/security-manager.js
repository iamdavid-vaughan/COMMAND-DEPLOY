...
`;
  ...
  async waitForInstanceReady(publicIp, connectOptions, deployLogger, maxRetries = 10, baseDelay = 10000) {
    ...
    // If we get here, all attempts and fallback failed
    const errorMessage = `SSH connection failed after ${maxRetries} attempts on port ${connectOptions.port}${connectOptions.fallbackPort ? ` and fallback port ${connectOptions.fallbackPort}` : ''}. Last error: ${lastError?.message || 'Unknown error'}`;
