...
const { DomainDetectionService } = require('./domain-detection');

class EnhancedSSLService {
  constructor(sshService) {
    this.sshService = sshService;
    this.dnsProviderService = new DNSProviderService();
    this.domainDetectionService = new DomainDetectionService();
  }
  ...
  async generateHTTPCertificate(host, domains, email, sshOptions = {}) {
    ...
    try {
      ...
      // Filter redundant domains before building certbot command
      const filteredDomains = this.domainDetectionService._filterRedundantDomains(domains);
      
      if (filteredDomains.length !== domains.length) {
        const removedDomains = domains.filter(d => !filteredDomains.includes(d));
        logger.info(chalk.yellow(`🔧 Filtered redundant domains: ${removedDomains.join(', ')}`));
        logger.info(chalk.blue(`📋 Using domains: ${filteredDomains.join(', ')}`));
      }
      
      // Build certbot command with filtered domains
      const domainFlags = filteredDomains.map(domain => `-d ${domain}`).join(' ');
      ...
      const primaryDomain = filteredDomains[0];
      return {
        success: true,
  ...
  async generateDNSCertificate(host, domains, email, sshOptions = {}, config = null) {
    ...
    try {
      ...
      // Build certbot command with filtered domains for DNS challenge
      const domainFlags = filteredDomains.map(domain => `-d ${domain}`).join(' ');
  ...
  async configureMultiDomainNginxSSL(host, domainConfigs, appPort, certificatePath, privateKeyPath, sshOptions = {}, dryRun = false) {
    ...
    try {
      // Ensure DH parameters file exists with fallback
      const hasDhparam = await this.ensureDhparamFile(host, sshOptions);
  ...
  /**
   * Ensure dhparam file exists with robust generation and fallback
   */
  async ensureDhparamFile(host, sshOptions = {}) {
    try {
      // First check if dhparam file already exists
      try {
        await this.sshService.executeCommand(host, 'sudo test -f /etc/ssl/certs/dhparam.pem', sshOptions);
        logger.info(chalk.green('✅ Diffie-Hellman parameters file already exists'));
        return true;
      } catch (error) {
        // File doesn't exist, need to generate it
        logger.info(chalk.blue('🔐 Generating Diffie-Hellman parameters (this may take a few minutes)...'));
      }

      // Try to generate dhparam with timeout and better error handling
      try {
        // Create the directory if it doesn't exist
        await this.sshService.executeCommand(host, 'sudo mkdir -p /etc/ssl/certs', sshOptions);
        
        // Generate dhparam with 2048 bits (faster than 4096)
        await this.sshService.executeCommand(
          host, 
          'sudo timeout 300 openssl dhparam -out /etc/ssl/certs/dhparam.pem 2048', 
          sshOptions
        );
        ...
        logger.warn(chalk.yellow(`⚠️  Failed to generate dhparam file: ${dhparamError.message}`));
        logger.info(chalk.blue('🔄 Continuing without dhparam (SSL will still work securely)'));
        return false;
      }
    } catch (error) {
      logger.warn(chalk.yellow(`⚠️  Error checking dhparam file: ${error.message}`));
      logger.info(chalk.blue('🔄 Continuing without dhparam (SSL will still work securely)'));
      return false;
    }
  }
  ...
  generateMultiDomainNginxSSLConfig(domainConfigs, appPort, certificatePath, privateKeyPath, hasDhparam = false) {
    ...
    const dhparamLine = hasDhparam ? '    ssl_dhparam /etc/ssl/certs/dhparam.pem;' : '    # ssl_dhparam disabled - file not available';
