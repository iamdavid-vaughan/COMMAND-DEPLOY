...
class EnhancedSSLService {
  ...
  /**
   * Filter out domains that are redundant with a wildcard domain
   * Let's Encrypt rejects requests with both wildcard and individual subdomains
   * @param {Array} domains - Array of domain names
   * @returns {Array} Filtered array without redundant domains
   */
  filterRedundantDomains(domains) {
    // Find all wildcard domains
    const wildcardDomains = domains.filter(d => d.startsWith('*.'));

    if (wildcardDomains.length === 0) {
      // No wildcards, return all domains
      return domains;
    }

    // Extract base domains from wildcards (e.g., '*.focuswithfocal.com' -> 'focuswithfocal.com')
    const wildcardBases = wildcardDomains.map(w => w.substring(2));

    // Filter: keep wildcards, base domains, and www subdomains only
    // Remove individual subdomains that would be covered by wildcards
    const filtered = domains.filter(domain => {
      // Keep all wildcards
      if (domain.startsWith('*.')) return true;

      // Check each wildcard base to see if this domain is redundant
      for (const baseDomain of wildcardBases) {
        // Keep the base domain itself
        if (domain === baseDomain) return true;

        // Keep www subdomain (Let's Encrypt recommends explicit www even with wildcard)
        if (domain === `www.${baseDomain}`) return true;

        // Remove any other subdomains of this base (they're covered by wildcard)
        if (domain.endsWith(`.${baseDomain}`) && !domain.startsWith('*.')) {
          logger.info(chalk.gray(`  Filtering out ${domain} (already covered by wildcard *.${baseDomain})`));
          return false;
        }
      }

      // Keep domains that don't match any wildcard base
      return true;
    });

    if (filtered.length < domains.length) {
      logger.info(chalk.blue(`📋 Certificate will include: ${filtered.join(', ')}`));
    }
  ...
  async generateHTTPCertificate(host, domains, email, sshOptions = {}) {
    ...
    try {
      ...
      // Filter out redundant domains (e.g., subdomains when wildcard is present)
      const filteredDomains = this.filterRedundantDomains(domains);
  ...
  generateMultiDomainNginxSSLConfig(domainConfigs, appPort, certificatePath, privateKeyPath) {
    ...
    // Nginx uses .domain notation (not *.domain) for wildcard matching
    const processedServerNames = allDomains.map(domain => {
      if (domain.startsWith('*.')) {
        // For wildcard SSL cert, Nginx needs .domain notation (matches all subdomains)
        // plus explicit base domain
        const baseDomain = domain.substring(2);
