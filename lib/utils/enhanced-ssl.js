...
class EnhancedSSLService {
   ...
   */
  async generateSANCertificate(host, domainConfigs, email, sshOptions = {}, dryRun = false, config = null, useStaging = false) {
    if (dryRun) {
      ...
      // Use deduplicated domains (wildcards are put first by deduplicateDomainsWithWildcards)
    ...
    try {
      ...
      if (httpDomains.length > 0 && dnsDomains.length === 0) {
        ...
        certificateResult = await this.generateDNSCertificate(host, dnsDomains, email, sshOptions, config, useStaging);
        ...
        certificateResult = await this.generateDNSCertificate(host, allDomains, email, sshOptions, config, useStaging);
      } else {
  ...
  async generateHTTPCertificate(host, domains, email, sshOptions = {}) {
    ...
    try {
      ...
      // Determine the primary domain for certificate path
      // Use the deduplicated domains list (already computed above)
      let primaryDomain = deduplicatedDomains[0];
   ...
   * @param {boolean} useStaging - Use Let's Encrypt staging server (for testing, avoids rate limits)
   */
  async generateDNSCertificate(host, domains, email, sshOptions = {}, config = null, useStaging = false) {
      ...
      if (providerCheck.configured) {
        ...
        try {
          return await this.dnsProviderService.setupAutomaticDNS01(
            ...
            sshOptions,
            false, // dryRun
            useStaging // Pass through staging mode
          );
        } catch (error) {
          logger.error(chalk.red(`❌ Automatic DNS-01 failed: ${error.message}`));

          // In automated deployment, we should fail rather than fall back to manual mode
          // Manual mode requires interactive input which won't work in CI/CD or automated deployments
          logger.error(chalk.red('Cannot fall back to manual mode in automated deployment'));
          logger.info(chalk.cyan('💡 Troubleshooting steps:'));
          logger.info(chalk.gray('  1. Check if there are stale ACME challenge records in your DNS'));
          logger.info(chalk.gray('     Run: node utils/cleanup-acme-records.js'));
          logger.info(chalk.gray('  2. Verify your DNS provider credentials are correct'));
          logger.info(chalk.gray('  3. Ensure DNS propagation is complete (may take up to 10 minutes)'));
          logger.info(chalk.gray('  4. Check Let\'s Encrypt rate limits'));
    ...
    try {
      // Remove redundant domains covered by wildcards to avoid Let's Encrypt errors
      const deduplicatedDomains = this.deduplicateDomainsWithWildcards(domains);
   ...
   */
  async configureMultiDomainNginxSSL(host, domainConfigs, appPort, certificatePath, privateKeyPath, sshOptions = {}, dryRun = false, hasDhparam = false) {
    if (dryRun) {
    ...
    // NOTE: hasDhparam is now passed in from SSL certificate service
    // It's checked/generated BEFORE requesting certificates to avoid wasting Let's Encrypt rate limits
    try {
   ...
   */
  generateMultiDomainNginxSSLConfig(domainConfigs, appPort, certificatePath, privateKeyPath, hasDhparam = false) {
...
server {
    ...
    ${hasDhparam ? 'ssl_dhparam /etc/ssl/certs/dhparam.pem;' : '# DH parameters not available (SSL still secure without them)'}
