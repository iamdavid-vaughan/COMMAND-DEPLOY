...
class EnhancedSSLService {
  ...
  filterRedundantDomains(domains) {
    ...
    // Filter: keep wildcards and base domains ONLY
    // Remove ALL subdomains (including www) - wildcard covers them all
    const filtered = domains.filter(domain => {
      ...
      for (const baseDomain of wildcardBases) {
        // Keep the base domain itself (wildcard *.example.com does NOT cover example.com)
        if (domain === baseDomain) return true;

        // Remove ALL subdomains of this base (wildcard *.example.com covers www, api, etc.)
