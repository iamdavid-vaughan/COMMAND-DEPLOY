...
class EnhancedSSLService {
  ...
  filterRedundantDomains(domains) {
    ...
    if (!wildcardDomain) {
      // No wildcard, return all domains
      return domains;
    }

    // Extract base domain from wildcard (e.g., '*.focuswithfocal.com' -> 'focuswithfocal.com')
    const baseDomain = wildcardDomain.substring(2);

    // Filter: keep wildcard, base domain, and www subdomain only
    // Remove individual subdomains that would be covered by the wildcard
    const filtered = domains.filter(domain => {
      if (domain === wildcardDomain) return true; // Keep wildcard
      if (domain === baseDomain) return true; // Keep base domain
      if (domain === `www.${baseDomain}`) return true; // Keep www subdomain

      // Remove any other subdomains (they're covered by wildcard)
      if (domain.endsWith(`.${baseDomain}`) && !domain.startsWith('*.')) {
        logger.info(chalk.gray(`  Filtering out ${domain} (already covered by wildcard ${wildcardDomain})`));
        return false;
      }

      return true; // Keep anything else (shouldn't happen but be safe)
    });
