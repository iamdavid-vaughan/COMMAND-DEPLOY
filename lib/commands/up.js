...
class UpCommand {
  ...
  async execute(options = {}) {
    ...
    try {
      ...
      // NOTE: Do NOT load existing state here - causes security group reuse across deployments!
      // The deployment executor handles state management properly via project-specific directories.
      // Loading global state causes all SaaS deployments to share the same security groups.
      ...
      // Include ALL resource IDs so they're available to subsequent deployment phases
      return {
        ...deploymentState.resources.ec2Instance,
        keyPairName: deploymentState.resources.sshKey.keyPairName,
        privateKeyPath: deploymentState.resources.sshKey.privateKeyPath,
        securityGroupId: deploymentState.resources.securityGroup?.securityGroupId,
        s3BucketName: deploymentState.resources.s3Bucket?.bucketName
      };
