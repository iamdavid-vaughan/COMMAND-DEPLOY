  ...
  displaySecurityGroupStatus(sgStatus) {
    ...
    if (sgStatus.status === 'exists') {
      ...
      if (sgStatus.inboundRules && sgStatus.inboundRules.length > 0) {
        const openPorts = sgStatus.inboundRules
          .filter(rule => rule.fromPort === rule.toPort)
          .map(rule => rule.fromPort);

        // Check for security issues
        const port22Open = openPorts.includes(22);

        if (openPorts.length > 0) {
          Logger.info(`  Open Ports: ${openPorts.join(', ')}`);
        }

        // Show security warning if port 22 is open
        if (port22Open) {
          Logger.warning('  ⚠️  SECURITY WARNING: Port 22 (default SSH) is open!');
          Logger.info('      This port should be closed after SSH hardening.');
          Logger.info('      Run "focal-deploy close-port-22" to secure your instance.');
        }
      }
    } else {
