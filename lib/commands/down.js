...
class DownCommand {
  ...
  async deleteEC2Instance(instanceInfo, ec2Manager) {
    ...
    try {
      ...
      spinner.text = `EC2 instance terminating: ${instanceInfo.instanceId}`;

      // Wait for instance to be fully terminated before returning
      // This is critical to ensure security group can be deleted
      spinner.text = 'Waiting for instance to fully terminate...';
      await this.waitForInstanceTermination(instanceInfo.instanceId, ec2Manager);
   ...
   * Polls AWS API until instance state is 'terminated'
   */
  async waitForInstanceTermination(instanceId, ec2Manager, maxWaitTime = 300000) {
    const startTime = Date.now();
    const pollInterval = 5000; // Check every 5 seconds

    while (Date.now() - startTime < maxWaitTime) {
      try {
        const instanceInfo = await ec2Manager.getInstanceInfo(instanceId);

        if (instanceInfo.state === 'terminated') {
          Logger.info(`Instance ${instanceId} fully terminated`);
          return;
        }

        Logger.info(`Instance state: ${instanceInfo.state} - waiting...`);
        await new Promise(resolve => setTimeout(resolve, pollInterval));

      } catch (error) {
        // If instance not found, it's terminated
        if (error.name === 'InvalidInstanceID.NotFound') {
          Logger.info(`Instance ${instanceId} fully terminated`);
          return;
        }
    ...
    Logger.warning(`Instance termination wait timed out after ${maxWaitTime / 1000}s - proceeding anyway`);
  }
  ...
  async cleanupGitHubRepository(config, options) {
    try {
      ...
      // Support both wizard config formats: config.git.repository and config.git.url
      const repoUrl = config.git?.repository || config.git?.url || config.git?.remoteUrl;
      ...
      if (!repoName) {
        ...
        Logger.info(`Repository URL found: ${repoUrl}`);
        return;
      }
