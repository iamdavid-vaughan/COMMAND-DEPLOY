...
class EULAManager {
  ...
  async promptForAcceptance(showFullEULA = false) {
    ...
    if (showFullEULA) {
      ...
      logger.info(chalk.gray(`Full EULA: https://focal-deploy.com/eula or run with --show-eula\n`));
    }
    ...
    if (action === 'decline') {
      ...
      logger.info(chalk.gray('If you have questions, please contact: legal@focal-deploy.com\n'));
      return false;
    }
    ...
    if (action === 'accept') {
      ...
      if (confirmed) {
        ...
        logger.info(chalk.gray('You can review the EULA anytime at: https://focal-deploy.com/eula\n'));
        return true;
      } else {
  ...
  async requireAcceptance() {
    ...
    if (!accepted) {
      ...
      if (!result) {
        ...
        logger.info(chalk.gray('For questions, contact: legal@focal-deploy.com\n'));
        process.exit(1);
      }
    }
  ...
  async showStatus() {
    ...
    logger.info(chalk.gray(`Effective Date: ${this.eulaDate}`));
    logger.info(chalk.gray('Full EULA: https://focal-deploy.com/eula\n'));
  }
