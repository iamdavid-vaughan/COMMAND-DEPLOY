...
async function testSecurityHardening() {
  ...
  try {
    ...
    const responses = [
      ...
      '2847',   // Custom SSH port (updated to match default)
      'dvaughan', // Username
    ...
    // Step 3: Test SSH connection on new port 2847
    console.log('Step 3: Testing SSH connection on hardened port 2847...');
    ...
    await new Promise((resolve, reject) => {
      hardenedSshTest.on('close', (code) => {
        if (code === 0) {
          console.log('✅ Hardened SSH connection on port 2847 successful\n');
          resolve();
        } else {
          console.log('❌ Hardened SSH connection on port 2847 failed\n');
