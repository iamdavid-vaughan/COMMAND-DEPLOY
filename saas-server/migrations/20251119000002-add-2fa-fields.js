...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    try { await queryInterface.addColumn('users', 'twofa_enabled', {
      type: Sequelize.BOOLEAN,
      allowNull: false,
      defaultValue: false,
      comment: 'Whether 2FA/TOTP is enabled for this user'
    });
    } catch(e) { if (!e.message.includes('already exists')) throw e; } console.log('Added twofa_enabled column');

    // Add twofa_secret field
    try { await queryInterface.addColumn('users', 'twofa_secret', {
      type: Sequelize.TEXT,
      allowNull: true,
      comment: 'Encrypted TOTP secret for 2FA'
    });
    } catch(e) { if (!e.message.includes('already exists')) throw e; } console.log('Added twofa_secret column');

    // Add twofa_backup_codes field
    try { await queryInterface.addColumn('users', 'twofa_backup_codes', {
      type: Sequelize.JSONB,
      allowNull: false,
      defaultValue: [],
      comment: 'Encrypted backup recovery codes for 2FA'
    });
    } catch(e) { if (!e.message.includes('already exists')) throw e; } console.log('Added twofa_backup_codes column');

    // Add index for 2FA enabled users (for analytics)
    try { await queryInterface.addIndex('users', ['twofa_enabled'], {
      name: 'idx_users_twofa_enabled'
    });
    } catch(e) { if (!e.message.includes('already exists')) throw e; } console.log('Added index on twofa_enabled');
